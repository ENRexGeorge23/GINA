...
class DeclinedRequestStateScreenLoaded extends StatelessWidget {
  final Map<DateTime, List<AppointmentModel>> declinedRequests;
  const DeclinedRequestStateScreenLoaded({
    super.key,
  ...
  Widget build(BuildContext context) {
    final declineRequestStateBloc = context.read<DeclinedRequestStateBloc>();
    var dates = declinedRequests.keys.toList();
    return ListView.builder(
      padding: const EdgeInsets.symmetric(horizontal: 20.0),
      itemCount: declinedRequests.length,
      itemBuilder: (context, index) {
        final date = dates[index];
        final requestsOnDate = declinedRequests[date]!;
        return Column(
          ...
          children: [
            ...
            ...requestsOnDate.map(
              (request) => GestureDetector(
                onTap: () {
                  declineRequestStateBloc.add(
                    NavigateToDeclinedRequestDetailEvent(
                      appointment: request,
                    ),
                  );
                },
